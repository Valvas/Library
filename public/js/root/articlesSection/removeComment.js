...
function removeArticleCommentOpenPrompt(articleUuid, commentUuid)
{
  ...
  modalHeaderTitle      .innerText = commonStrings.root.news.homePage.removeComment.confirmationTitle;
  modalContentConfirm   .innerText = commonStrings.root.news.homePage.removeComment.confirmButton;
  modalContentCancel    .innerText = commonStrings.root.news.homePage.removeComment.cancelButton;

  modalContent          .innerHTML = `<div class="baseModalContentMessage">${commonStrings.root.news.homePage.removeComment.confirmationMessage}</div>`;
  ...
  {
    removeArticleCommentSendToServer(articleUuid, commentUuid);
  });

  modalContentCancel    .addEventListener('click', () =>
  {
    checkMessageTag('removeArticleCommentError');
...
function removeArticleCommentSendToServer(articleUuid, commentUuid)
{
  ...
  $.ajax(
  {
    method: 'DELETE', timeout: 10000, dataType: 'JSON', data: { commentUuid: commentUuid, articleUuid: articleUuid }, url: '/queries/root/news/remove-comment-on-article', success: () => {},
    error: (xhr, status, error) =>
    {
      ...
      displayError(xhr.responseJSON.message, xhr.responseJSON.detail, 'removeArticleCommentError') :
      displayError('Une erreur est survenue, veuillez réessayer plus tard', null, 'removeArticleCommentError');
    }
  ...
  }).done((result) =>
  {
    ...
    displaySuccess(result.message, null, 'removeArticleCommentSuccess');
  });
}
